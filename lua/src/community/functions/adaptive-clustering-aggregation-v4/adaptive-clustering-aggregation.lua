...
- **Plasing**: Waar in die kluster elke uitvoerdatapunt geplaas word:
  - Venster-einde: By die mees onlangse datapunt in die kluster (verstek)
  - Venster-middelpunt: By die tydelike middelpunt van die kluster
  - Venster-begin: By die oudste datapunt in die kluster
  	]],
  	["sq"] = [[
...
- **Pozicionimi**: Ku të vendoset çdo pikë dalëse e të dhënave në klaster:
  - Fundi i dritares: Te pika më e fundit e të dhënave në klaster (parazgjedhje)
  - Mesii i dritares: Në qendrën kohore të klasterit
  - Fillimi i dritares: Te pika më e vjetër e të dhënave në klaster
  	]],
  	["am"] = [[
...
- **አቀማመጥ**፦ እያንዳንዱ የውጤት የውሂብ ነጥብ በስብስቡ ውስጥ የሚቀመጥበት ቦታ፦
  - የመስኮት መጨረሻ፦ በስብስቡ ውስጥ ባለው የቅርብ ጊዜ የውሂብ ነጥብ (ነባሪ)
  - የመስኮት መሃል፦ በስብስቡ የጊዜ መሃል
  - የመስኮት መጀመሪያ፦ በስብስቡ ውስጥ ባለው እጅግ የቆየ የውሂብ ነጥብ
  	]],
  	["hy"] = [[
...
- **Տեղադրում**․ կլաստերի որ հատվածում տեղադրել յուրաքանչյուր ելքային տվյալակետը՝
  - Window End՝ կլաստերի ամենավերջին տվյալակետի պահին (կանխադրված)
  - Window Midpoint՝ կլաստերի ժամանակային կենտրոնում
  - Window Start՝ կլաստերի ամենահին տվյալակետի պահին
  	]],
  	["az"] = [[
...
- **Yerləşdirmə**: Hər çıxış məlumat nöqtəsinin klasterdə yerləşdiriləcəyi yer:
  - Window End: Klasterdəki ən son məlumat nöqtəsində (standart)
  - Window Midpoint: Klasterin zaman mərkəzində
  - Window Start: Klasterdəki ən köhnə məlumat nöqtəsində
  	]],
  	["bn"] = [[
...
- **অবস্থান**: ক্লাস্টারের কোথায় প্রতিটি আউটপুট ডেটা পয়েন্ট রাখা হবে:
  - Window End: ক্লাস্টারের সর্বশেষ ডেটা পয়েন্টে (ডিফল্ট)
  - Window Midpoint: ক্লাস্টারের সময়গত কেন্দ্রে
  - Window Start: ক্লাস্টারের প্রাচীনতম ডেটা পয়েন্টে
  	]],
  	["eu"] = [[
...
- **Kokapena**: Multzoan irteerako datu-puntu bakoitza non kokatu:
  - Leihoaren amaiera: Multzoko azken datu-puntuan (lehenetsia)
  - Leihoaren erdiko puntua: Multzoaren erdigune kronologikoan
  - Leihoaren hasiera: Multzoko datu-puntu zaharrenean
  	]],
  	["be"] = [[
...
- **Размяшчэнне**: дзе ў кластары размясціць кожную выходную кропку даных:
  - Канец акна: у самай позняй кропцы даных у кластары (па змаўчанні)
  - Сярэдзіна акна: у часовым цэнтры кластара
  - Пачатак акна: у самай ранняй кропцы даных у кластары
  	]],
  	["bg"] = [[
...
- **Позициониране**: Къде в клъстера да бъде поставена всяка изходна точка от данни:
  - Край на прозореца: При най-новата точка от данни в клъстера (по подразбиране)
  - Среда на прозореца: Във времевия център на клъстера
  - Начало на прозореца: При най-старата точка от данни в клъстера
  	]],
  	["my"] = [[
...
- **နေရာချထားမှု**: ထွက်လာသော ဒေတာမှတ်တစ်ခုစီကို အစုအတွင်း မည်သည့်နေရာတွင် ထားမည်နည်း:
  - Window End: အစုအတွင်း နောက်ဆုံးဒေတာမှတ်နေရာ (မူလ)
  - Window Midpoint: အစု၏ အချိန်အလယ်ဗဟိုနေရာ
  - Window Start: အစုအတွင်း အစောဆုံးဒေတာမှတ်နေရာ
  	]],
  	["ca"] = [[
...
- **Posicionament**: On col·locar cada punt de dades de sortida dins del clúster:
  - Final de la finestra: Al punt de dades més recent del clúster (per defecte)
  - Punt mitjà de la finestra: Al centre temporal del clúster
  - Inici de la finestra: Al punt de dades més antic del clúster
  	]],
  	["zh-Hans"] = [[
...
- **放置位置**：每个输出数据点在聚类中的放置位置：
  - 窗口结束：位于聚类中最新的数据点处（默认）
  - 窗口中点：位于聚类的时间中心
  - 窗口开始：位于聚类中最早的数据点处
  	]],
  	["zh-Hant"] = [[
...
- **Smještaj**: Gdje u klaster smjestiti svaku izlaznu podatkovnu točku:
  - Kraj prozora: Na najnoviju podatkovnu točku u klasteru (zadano)
  - Sredina prozora: U vremensko središte klastera
  - Početak prozora: Na najstariju podatkovnu točku u klasteru
  	]],
  	["cs"] = [[
...
- **Umístění**: Kam ve shluku umístit každý výstupní datový bod:
  - Konec okna: K nejnovějšímu datovému bodu ve shluku (výchozí)
  - Střed okna: Do časového středu shluku
  - Začátek okna: K nejstaršímu datovému bodu ve shluku
  	]],
  	["da"] = [[
...
- **Placering**: Hvor i klyngen hvert outputdatapunkt placeres:
  - Vinduets slutning: Ved det seneste datapunkt i klyngen (standard)
  - Vinduets midtpunkt: Ved klyngens tidsmæssige centrum
  - Vinduets begyndelse: Ved det ældste datapunkt i klyngen
  	]],
  	["nl"] = [[
...
- **Plaatsing**: Waar elk uitvoergegevenspunt in de cluster wordt geplaatst:
  - Einde van venster: Bij het meest recente gegevenspunt in de cluster (standaard)
  - Midden van venster: In het tijdsmidden van de cluster
  - Begin van venster: Bij het oudste gegevenspunt in de cluster
  	]],
  	["et"] = [[
...
- **Paigutus**: kuhu klastris paigutada iga väljundi andmepunkt:
  - Akna lõpp: klastri kõige uuema andmepunkti juurde (vaikimisi)
  - Akna keskpunkt: klastri ajalisse keskpunkti
  - Akna algus: klastri vanima andmepunkti juurde
  	]],
  	["fil"] = [[
...
- **Placement**: Kung saan ilalagay ang bawat output data point sa kumpol:
  - Window End: Sa pinakabagong data point sa kumpol (default)
  - Window Midpoint: Sa gitnang oras ng kumpol
  - Window Start: Sa pinakamatandang data point sa kumpol
  	]],
  	["fi"] = [[
...
- **Sijoittelu**: Mihin kohtaan klusteria kukin tuloksena syntyvä datapiste sijoitetaan:
  - Aikaikkunan loppu: Klusterin uusimman datapisteen kohdalle (oletus)
  - Aikaikkunan keskipiste: Klusterin ajalliseen keskikohtaan
  - Aikaikkunan alku: Klusterin vanhimman datapisteen kohdalle
  	]],
  	["fr"] = [[
...
- **Positionnement** : Position de chaque point de données produit dans le groupe :
  - Fin de la fenêtre : Au point de données le plus récent du groupe (par défaut)
  - Milieu de la fenêtre : Au centre temporel du groupe
  - Début de la fenêtre : Au point de données le plus ancien du groupe
  	]],
  	["gl"] = [[
...
- **Posición**: Onde colocar cada punto de datos de saída no grupo:
  - Final da xanela: No punto de datos máis recente do grupo (predeterminado)
  - Punto medio da xanela: No centro temporal do grupo
  - Inicio da xanela: No punto de datos máis antigo do grupo
  	]],
  	["ka"] = [[
...
- **განთავსება**: კლასტერის რომელ ადგილას განთავსდეს თითოეული გამოტანილი მონაცემის წერტილი:
  - Window End: კლასტერის უახლეს მონაცემთა წერტილზე (ნაგულისხმევი)
  - Window Midpoint: კლასტერის დროით შუა წერტილში
  - Window Start: კლასტერის უძველეს მონაცემთა წერტილზე
  	]],
  	["de"] = [[
...
- **Positionierung**: Wo jeder Ausgabedatenpunkt im Cluster platziert wird:
  - Fensterende: Beim neuesten Datenpunkt im Cluster (Standard)
  - Fenstermitte: In der zeitlichen Mitte des Clusters
  - Fensteranfang: Beim ältesten Datenpunkt im Cluster
  	]],
  	["el"] = [[
...
- **Τοποθέτηση**: Πού στη συστάδα τοποθετείται κάθε σημείο δεδομένων εξόδου:
  - Τέλος παραθύρου: Στο πιο πρόσφατο σημείο δεδομένων της συστάδας (προεπιλογή)
  - Μέσο παραθύρου: Στο χρονικό κέντρο της συστάδας
  - Αρχή παραθύρου: Στο παλαιότερο σημείο δεδομένων της συστάδας
  	]],
  	["gu"] = [[
...
- **સ્થાનનિર્ધારણ**: દરેક આઉટપુટ ડેટા પોઇન્ટને ક્લસ્ટરમાં ક્યાં મૂકવો:
  - Window End: ક્લસ્ટરના સૌથી તાજેતરના ડેટા પોઇન્ટ પર (ડિફૉલ્ટ)
  - Window Midpoint: ક્લસ્ટરના સમયગાળાના મધ્યબિંદુ પર
  - Window Start: ક્લસ્ટરના સૌથી જૂના ડેટા પોઇન્ટ પર
  	]],
  	["hi"] = [[
...
- **स्थान**: प्रत्येक आउटपुट डेटा पॉइंट को क्लस्टर में कहाँ रखना है:
  - विंडो का अंत: क्लस्टर के सबसे हाल के डेटा पॉइंट पर (डिफ़ॉल्ट)
  - विंडो का मध्यबिंदु: क्लस्टर के समयगत केंद्र पर
  - विंडो की शुरुआत: क्लस्टर के सबसे पुराने डेटा पॉइंट पर
  	]],
  	["hu"] = [[
...
- **Elhelyezés**: Az egyes kimeneti adatpontok klaszteren belüli elhelyezése:
  - Ablak vége: A klaszter legutóbbi adatpontjánál (alapértelmezett)
  - Ablak közepe: A klaszter időbeli középpontjánál
  - Ablak eleje: A klaszter legrégebbi adatpontjánál
  	]],
  	["is"] = [[
...
- **Staðsetning**: Hvar í klasanum hver úttaksgagnapunktur er staðsettur:
  - Lok glugga: Við nýjasta gagnapunkt klasans (sjálfgefið)
  - Miðja glugga: Í tímalegri miðju klasans
  - Upphaf glugga: Við elsta gagnapunkt klasans
  	]],
  	["id"] = [[
...
- **Penempatan**: Lokasi penempatan setiap titik data keluaran dalam klaster:
  - Akhir Jendela: Pada titik data terbaru dalam klaster (default)
  - Titik Tengah Jendela: Pada pusat waktu klaster
  - Awal Jendela: Pada titik data tertua dalam klaster
  	]],
  	["it"] = [[
...
- **Posizionamento**: dove collocare ogni punto dati di output nel cluster:
  - Fine della finestra: sul punto dati più recente del cluster (predefinito)
  - Punto centrale della finestra: al centro temporale del cluster
  - Inizio della finestra: sul punto dati più vecchio del cluster
  	]],
  	["ja"] = [[
...
- **配置**: 各出力データポイントをクラスタ内のどこに配置するか:
  - Window End: クラスタ内の最も新しいデータポイント（デフォルト）
  - Window Midpoint: クラスタの時間的な中央
  - Window Start: クラスタ内の最も古いデータポイント
  	]],
  	["kn"] = [[
...
- **ಸ್ಥಾನ**: ಪ್ರತಿ ಔಟ್‌ಪುಟ್ ಡೇಟಾ ಬಿಂದುವನ್ನು ಕ್ಲಸ್ಟರ್‌ನಲ್ಲಿ ಇರಿಸುವ ಸ್ಥಳ:
  - Window End: ಕ್ಲಸ್ಟರ್‌ನ ಅತ್ಯಂತ ಇತ್ತೀಚಿನ ಡೇಟಾ ಬಿಂದುವಿನಲ್ಲಿ (ಡೀಫಾಲ್ಟ್)
  - Window Midpoint: ಕ್ಲಸ್ಟರ್‌ನ ಸಮಯದ ಮಧ್ಯಭಾಗದಲ್ಲಿ
  - Window Start: ಕ್ಲಸ್ಟರ್‌ನ ಅತಿ ಹಳೆಯ ಡೇಟಾ ಬಿಂದುವಿನಲ್ಲಿ
  	]],
  	["kk"] = [[
...
- **Орналастыру**: Әр шығыс дерек нүктесін кластердің қай жеріне орналастыру керек:
  - Window End: Кластердегі ең соңғы дерек нүктесінде (әдепкі)
  - Window Midpoint: Кластердің уақыт бойынша ортасында
  - Window Start: Кластердегі ең ескі дерек нүктесінде
  	]],
  	["km"] = [[
...
- **ទីតាំង**៖ កន្លែងដាក់ចំណុចទិន្នន័យលទ្ធផលនីមួយៗក្នុងក្រុម៖
  - ចុងបង្អួច៖ នៅចំណុចទិន្នន័យថ្មីបំផុតក្នុងក្រុម (លំនាំដើម)
  - ចំណុចកណ្តាលបង្អួច៖ នៅកណ្តាលតាមពេលវេលារបស់ក្រុម
  - ដើមបង្អួច៖ នៅចំណុចទិន្នន័យចាស់បំផុតក្នុងក្រុម
  	]],
  	["ko"] = [[
...
- **배치**: 각 출력 데이터 포인트를 클러스터 내 어디에 배치할지:
  - Window End: 클러스터에서 가장 최근 데이터 포인트 위치(기본값)
  - Window Midpoint: 클러스터의 시간상 중앙
  - Window Start: 클러스터에서 가장 오래된 데이터 포인트 위치
  	]],
  	["ky"] = [[
...
- **Жайгаштыруу**: Ар бир чыгаруу маалымат чекитин кластерде жайгаштыруу орду:
  - Терезенин аягы: Кластердеги эң акыркы маалымат чекитинде (демейки)
  - Терезенин ортосу: Кластердин убакыттык ортосунда
  - Терезенин башы: Кластердеги эң эски маалымат чекитинде
  	]],
  	["lo"] = [[
...
- **ຕຳແໜ່ງ**: ບ່ອນວາງຈຸດຂໍ້ມູນຜົນລັບແຕ່ລະຈຸດໃນກຸ່ມ:
  - ທ້າຍໜ້າຕ່າງ: ຢູ່ຈຸດຂໍ້ມູນຫຼ້າສຸດໃນກຸ່ມ (ຄ່າເລີ່ມຕົ້ນ)
  - ຈຸດກາງໜ້າຕ່າງ: ຢູ່ກາງດ້ານເວລາຂອງກຸ່ມ
  - ຕົ້ນໜ້າຕ່າງ: ຢູ່ຈຸດຂໍ້ມູນເກົ່າສຸດໃນກຸ່ມ
  	]],
  	["lv"] = [[
...
- **Novietojums**: Kur klasterī novietot katru izvades datu punktu:
  - Loga beigas: Pie jaunākā klastera datu punkta (noklusējums)
  - Loga viduspunkts: Klastera laika vidū
  - Loga sākums: Pie vecākā klastera datu punkta
  	]],
  	["lt"] = [[
...
- **Vieta**: Kur grupėje patalpinti kiekvieną išvesties duomenų tašką:
  - Lango pabaiga: Ties naujausiu grupės duomenų tašku (numatyta)
  - Lango vidurys: Laikiniame grupės centre
  - Lango pradžia: Ties seniausiu grupės duomenų tašku
  	]],
  	["mk"] = [[
...
- **Поставување**: Каде во кластерот да се постави секоја излезна точка на податоци:
  - Крај на прозорецот: Кај најновата точка на податоци во кластерот (стандардно)
  - Средина на прозорецот: Во временскиот центар на кластерот
  - Почеток на прозорецот: Кај најстарата точка на податоци во кластерот
  	]],
  	["ms"] = [[
...
- **Penempatan**: Tempat untuk meletakkan setiap titik data output dalam kelompok:
  - Hujung Tetingkap: Pada titik data paling terkini dalam kelompok (lalai)
  - Titik Tengah Tetingkap: Pada pusat masa kelompok
  - Permulaan Tetingkap: Pada titik data paling lama dalam kelompok
  	]],
  	["ml"] = [[
...
- **Placement**: ഓരോ ഔട്ട്പുട്ട് ഡാറ്റാ പോയിന്റും ക്ലസ്റ്ററിൽ സ്ഥാപിക്കേണ്ട സ്ഥാനം:
  - Window End: ക്ലസ്റ്ററിലെ ഏറ്റവും പുതിയ ഡാറ്റാ പോയിന്റിൽ (സ്ഥിരസ്ഥിതി)
  - Window Midpoint: ക്ലസ്റ്ററിന്റെ സമയമധ്യത്തിൽ
  - Window Start: ക്ലസ്റ്ററിലെ ഏറ്റവും പഴയ ഡാറ്റാ പോയിന്റിൽ
  	]],
  	["mr"] = [[
...
- **स्थान**: प्रत्येक आउटपुट डेटा बिंदू क्लस्टरमध्ये कुठे ठेवायचा:
  - विंडोचा शेवट: क्लस्टरमधील सर्वात अलीकडील डेटा बिंदूवर (डीफॉल्ट)
  - विंडोचा मध्यबिंदू: क्लस्टरच्या कालावधीच्या मध्यभागी
  - विंडोची सुरुवात: क्लस्टरमधील सर्वात जुना डेटा बिंदूवर
  	]],
  	["mn"] = [[
...
- **Байрлал**: Гаралтын өгөгдлийн цэгийг бүлгийн хаана байрлуулах:
  - Window End: Бүлгийн хамгийн сүүлийн өгөгдлийн цэг дээр (анхдагч)
  - Window Midpoint: Бүлгийн хугацааны дунд цэгт
  - Window Start: Бүлгийн хамгийн эхний өгөгдлийн цэг дээр
  	]],
  	["ne"] = [[
...
- **स्थान**: प्रत्येक आउटपुट डेटा बिन्दुलाई क्लस्टरमा राख्ने स्थान:
  - Window End: क्लस्टरको सबैभन्दा पछिल्लो डेटा बिन्दुमा (पूर्वनिर्धारित)
  - Window Midpoint: क्लस्टरको समयगत केन्द्रमा
  - Window Start: क्लस्टरको सबैभन्दा पुरानो डेटा बिन्दुमा
  	]],
  	["no"] = [[
...
- **Plassering**: Hvor i klyngen hvert utgående datapunkt skal plasseres:
  - Slutt på vinduet: Ved det nyeste datapunktet i klyngen (standard)
  - Midtpunkt i vinduet: Ved klyngens tidsmessige sentrum
  - Start på vinduet: Ved det eldste datapunktet i klyngen
  	]],
  	["pl"] = [[
...
- **Położenie**: Miejsce umieszczenia każdego wyjściowego punktu danych w klastrze:
  - Koniec okna: Przy najnowszym punkcie danych w klastrze (domyślnie)
  - Środek okna: W czasowym środku klastra
  - Początek okna: Przy najstarszym punkcie danych w klastrze
  	]],
  	["pt"] = [[
...
- **Posicionamento**: Onde colocar cada ponto de dados de saída no grupo:
  - Fim da janela: No ponto de dados mais recente do grupo (predefinição)
  - Ponto médio da janela: No centro temporal do grupo
  - Início da janela: No ponto de dados mais antigo do grupo
  	]],
  	["pa"] = [[
...
- **ਸਥਾਪਨਾ**: ਹਰੇਕ ਆਉਟਪੁੱਟ ਡਾਟਾ ਪੁਆਇੰਟ ਨੂੰ ਕਲੱਸਟਰ ਵਿੱਚ ਕਿੱਥੇ ਰੱਖਣਾ ਹੈ:
  - Window End: ਕਲੱਸਟਰ ਦੇ ਸਭ ਤੋਂ ਨਵੇਂ ਡਾਟਾ ਪੁਆਇੰਟ ਉੱਤੇ (ਡਿਫਾਲਟ)
  - Window Midpoint: ਕਲੱਸਟਰ ਦੇ ਸਮੇਂਕ ਕੇਂਦਰ ਉੱਤੇ
  - Window Start: ਕਲੱਸਟਰ ਦੇ ਸਭ ਤੋਂ ਪੁਰਾਣੇ ਡਾਟਾ ਪੁਆਇੰਟ ਉੱਤੇ
  	]],
  	["ro"] = [[
...
- **Poziționare**: Unde să fie plasat fiecare punct de date rezultat în cluster:
  - Sfârșitul ferestrei: La cel mai recent punct de date din cluster (implicit)
  - Mijlocul ferestrei: În centrul temporal al clusterului
  - Începutul ferestrei: La cel mai vechi punct de date din cluster
  	]],
  	["rm"] = [[
...
- **Posiziun**: Nua plazzar mintga punct da datas d’output en il cluster:
  - Fin da la fanestra: Al punct da datas il pli recent en il cluster (default)
  - Center da la fanestra: Al center temporal dal cluster
  - Cumenzament da la fanestra: Al punct da datas il pli vegl en il cluster
  	]],
  	["ru"] = [[
...
- **Размещение**: Где размещать каждую выходную точку данных в кластере:
  - Конец окна: В момент самой поздней точки данных кластера (по умолчанию)
  - Середина окна: В центре временного интервала кластера
  - Начало окна: В момент самой ранней точки данных кластера
  	]],
  	["sr"] = [[
...
- **Položaj**: Mesto u klasteru na koje se postavlja svaka izlazna tačka podataka:
  - Kraj prozora: Na najnoviju tačku podataka u klasteru (podrazumevano)
  - Sredina prozora: Na vremensku sredinu klastera
  - Početak prozora: Na najstariju tačku podataka u klasteru
  	]],
  	["si"] = [[
...
- **ස්ථානගත කිරීම**: එක් එක් ප්‍රතිදාන දත්ත ලක්ෂ්‍යය පොකුර තුළ තැබිය යුතු ස්ථානය:
  - කවුළු අවසානය: පොකුරේ නවතම දත්ත ලක්ෂ්‍යයේ (පෙරනිමිය)
  - කවුළු මධ්‍ය ලක්ෂ්‍යය: පොකුරේ කාලානුක්‍රමික මධ්‍යයේ
  - කවුළු ආරම්භය: පොකුරේ පැරණිතම දත්ත ලක්ෂ්‍යයේ
  	]],
  	["sk"] = [[
...
- **Umiestnenie**: Kam v zhluku umiestniť každý výstupný údajový bod:
  - Koniec okna: Pri najnovšom údajovom bode v zhluku (predvolené)
  - Stred okna: V časovom strede zhluku
  - Začiatok okna: Pri najstaršom údajovom bode v zhluku
  	]],
  	["sl"] = [[
...
- **Postavitev**: Kam v skupino se postavi vsaka izhodna podatkovna točka:
  - Konec okna: Ob najnovejši podatkovni točki v skupini (privzeto)
  - Sredina okna: Na časovno sredino skupine
  - Začetek okna: Ob najstarejši podatkovni točki v skupini
  	]],
  	["es"] = [[
...
- **Ubicación**: Dónde colocar cada punto de datos de salida en el grupo:
  - Fin de la ventana: En el punto de datos más reciente del grupo (predeterminado)
  - Punto medio de la ventana: En el centro temporal del grupo
  - Inicio de la ventana: En el punto de datos más antiguo del grupo
  	]],
  	["sw"] = [[
...
- **Mahali**: Mahali pa kuweka kila nukta ya data ya matokeo ndani ya kundi:
  - Mwisho wa Dirisha: Kwenye nukta ya data ya hivi karibuni katika kundi (chaguo-msingi)
  - Katikati ya Dirisha: Katikati ya muda ya kundi
  - Mwanzo wa Dirisha: Kwenye nukta ya data ya zamani zaidi katika kundi
  	]],
  	["sv"] = [[
...
- **Placering**: Var i klustret varje utgående datapunkt placeras:
  - Fönstrets slut: Vid den senaste datapunkten i klustret (standard)
  - Fönstrets mittpunkt: Vid klustrets tidsmässiga mittpunkt
  - Fönstrets början: Vid den äldsta datapunkten i klustret
  	]],
  	["ta"] = [[
...
- **நிலைநிறுத்தல்**: ஒவ்வொரு வெளியீட்டுத் தரவுப் புள்ளியையும் குழுவில் வைக்கும் இடம்:
  - Window End: குழுவில் உள்ள சமீபத்திய தரவுப் புள்ளியில் (இயல்புநிலை)
  - Window Midpoint: குழுவின் கால மையத்தில்
  - Window Start: குழுவில் உள்ள பழைய தரவுப் புள்ளியில்
  	]],
  	["te"] = [[
...
- **స్థానం**: ప్రతి అవుట్‌పుట్ డేటా పాయింట్‌ను క్లస్టర్‌లో ఎక్కడ ఉంచాలి:
  - Window End: క్లస్టర్‌లోని అత్యంత ఇటీవలి డేటా పాయింట్ వద్ద (డిఫాల్ట్)
  - Window Midpoint: క్లస్టర్ యొక్క కాలకేంద్రంలో
  - Window Start: క్లస్టర్‌లోని ప్రాచీన డేటా పాయింట్ వద్ద
  	]],
  	["th"] = [[
...
- **ตำแหน่ง**: ตำแหน่งที่จะวางจุดข้อมูลผลลัพธ์แต่ละจุดในคลัสเตอร์:
  - สิ้นสุดช่วงเวลา: ที่จุดข้อมูลล่าสุดในคลัสเตอร์ (ค่าเริ่มต้น)
  - จุดกึ่งกลางช่วงเวลา: ที่กึ่งกลางตามเวลาของคลัสเตอร์
  - เริ่มต้นช่วงเวลา: ที่จุดข้อมูลเก่าที่สุดในคลัสเตอร์
  	]],
  	["tr"] = [[
...
- **Yerleştirme**: Her çıktı veri noktasının küme içinde yerleştirileceği konum:
  - Pencere Sonu: Kümedeki en yeni veri noktasında (varsayılan)
  - Pencere Ortası: Kümenin zamansal merkezinde
  - Pencere Başlangıcı: Kümedeki en eski veri noktasında
  	]],
  	["uk"] = [[
...
- **Розміщення**: Де в кластері розміщувати кожну вихідну точку даних:
  - Кінець вікна: Біля найновішої точки даних у кластері (типово)
  - Середина вікна: У часовому центрі кластера
  - Початок вікна: Біля найстарішої точки даних у кластері
  	]],
  	["vi"] = [[
...
- **Vị trí**: Vị trí đặt mỗi điểm dữ liệu đầu ra trong cụm:
  - Cuối cửa sổ: Tại điểm dữ liệu gần đây nhất trong cụm (mặc định)
  - Trung điểm cửa sổ: Tại trung tâm thời gian của cụm
  - Đầu cửa sổ: Tại điểm dữ liệu cũ nhất trong cụm
  	]],
  },
  config = {
    enum {
