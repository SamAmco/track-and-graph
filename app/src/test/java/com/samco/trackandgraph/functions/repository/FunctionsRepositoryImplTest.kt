...
class FunctionsRepositoryImplTest {
    ...
    @Test
    fun `fetchFunctions happy path - successfully fetches, verifies signature, and parses functions`() = runTest {
        // Given - Load test data from resources
        val luaScript = javaClass.getResourceAsStream("/functions-catalog/community-functions.lua")
            ?.bufferedReader()
            ?.readText()
            ?: throw IllegalStateException("Could not read test community-functions.lua")

        val publicKey = javaClass.getResourceAsStream("/functions-catalog/debug-20251010T194102Z.pub")
            ?.bufferedReader()
            ?.readText()
    ...
    @Test
    fun `fetchFunctions signature verification fails - throws SignatureVerificationException`() = runTest {
    ...
    fun `fetchFunctions service throws exception - returns empty list`() = runTest {
        ...
        // Then
        assertEquals("Should return empty list on service failure", emptyList<LuaFunctionMetadata>(), result)
    ...
    @Test
    fun `fetchFunctions lua engine throws exception - returns empty list and releases VM`() = runTest {
        ...
        // Then
        assertEquals("Should return empty list on Lua parsing failure", emptyList<LuaFunctionMetadata>(), result)
