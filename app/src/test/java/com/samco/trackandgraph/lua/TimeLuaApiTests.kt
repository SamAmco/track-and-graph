...
class TimeLuaApiTests : LuaEngineImplTest() {
    ...
    fun `shift timestamp forward by duration`() = testLuaEngine(
        ...
        """
            ...
            local shifted = tng.time.shift(timestamp, tng.time.D_DAY)
            return {
                type = tng.graph.TEXT,
    ...
    fun `shift date forward by duration and amount`() = testLuaEngine(
        ...
        """
            ...
            local shifted = tng.time.shift(date, tng.time.D_DAY, 2)
            return {
                type = tng.graph.TEXT,
                data = shifted.timestamp
            }
    ...
    fun `shift timestamp backward by duration and amount`() = testLuaEngine(
        ...
        """
            local timestamp = { timestamp = 1743202801000 }
            local shifted = tng.time.shift(timestamp, tng.time.D_DAY, -2)
            return {
                type = tng.graph.TEXT,
                data = shifted.timestamp
            }
    ...
    fun `shift date backward by duration`() = testLuaEngine(
        ...
        """
            local date = { year = 2025, month = 3, day = 29, sec = 1, zone = "Europe/Berlin" }
            local shifted = tng.time.shift(date, tng.time.D_DAY, -1)
            return {
                type = tng.graph.TEXT,
    ...
    fun `shift timestamp forward by period`() = testLuaEngine(
        ...
        """
            ...
            local shifted = tng.time.shift(timestamp, tng.time.P_DAY, 2)
            return {
                type = tng.graph.TEXT,
                data = table.concat({ shifted.timestamp, shifted.offset }, " ")
            }
    ...
    fun `shift date backward by period and amount`() = testLuaEngine(
        ...
        """
            local date = { year = 2025, month = 3, day = 29, sec = 1, zone = "Europe/Berlin" }
            local shifted = tng.time.shift(date, tng.time.P_DAY, -2)
            return {
                type = tng.graph.TEXT,
                data = table.concat({ shifted.timestamp, shifted.offset }, " ")
            }
