...
internal class AlarmSchedulerImplTest {
    ...
    fun `schedule next when today is enabled and reminder time is later today returns today`() {
        ...
        timeProvider.currentTime = ZonedDateTime.of(2024, 1, 2, 10, 0, 0, 0, ZoneId.of("UTC")) // Tuesday
    ...
    fun `schedule next when today is enabled but reminder time already passed returns next occurrence`() {
        ...
        timeProvider.currentTime = ZonedDateTime.of(2024, 1, 2, 16, 0, 0, 0, ZoneId.of("UTC")) // Tuesday 4:00 PM
        ...
        val expected = ZonedDateTime.of(2024, 1, 4, 14, 0, 0, 0, ZoneId.of("UTC")).toInstant() // Thursday
        assertEquals("Should schedule for next occurrence when today's time has passed", expected, result)
    }
    ...
    fun `schedule next with today not enabled returns next enabled day`() {
        ...
        // VERIFY - Should schedule for Thursday at 2:00 PM
        val expected = ZonedDateTime.of(2024, 1, 4, 14, 0, 0, 0, ZoneId.of("UTC")).toInstant() // Thursday
        assertEquals("Should schedule for next enabled day when today is not enabled", expected, result)
    }
    ...
    fun `schedule next with multiple days enabled returns earliest next occurrence`() {
        ...
        val reminder = reminderFixture.copy(
            time = LocalTime.of(14, 0), // 2:00 PM
            checkedDays = CheckedDays.none().copy(wednesday = true, friday = true) // Wed & Fri enabled
        )
        ...
        val expected = ZonedDateTime.of(2024, 1, 3, 14, 0, 0, 0, ZoneId.of("UTC")).toInstant() // Wednesday
        assertEquals("Should schedule for earliest next occurrence when multiple days are enabled", expected, result)
    }
    ...
    fun `schedule next wraps to next week when needed`() {
        ...
        timeProvider.currentTime = ZonedDateTime.of(2024, 1, 5, 10, 0, 0, 0, ZoneId.of("UTC")) // Friday
        ...
        val expected = ZonedDateTime.of(2024, 1, 8, 14, 0, 0, 0, ZoneId.of("UTC")).toInstant() // Next Monday
        assertEquals("Should wrap to next week when no enabled days remain this week", expected, result)
    }
    ...
    fun `schedule next handles timezone correctly`() {
        ...
        timeProvider.currentTime = ZonedDateTime.of(2024, 1, 2, 10, 0, 0, 0, estZone) // Tuesday 10:00 AM EST
    ...
    fun `schedule next adds buffer to avoid race conditions`() {
        ...
        timeProvider.currentTime = ZonedDateTime.of(2024, 1, 2, 14, 0, 0, 0, ZoneId.of("UTC")) // Tuesday 2:00 PM exactly
        
        val reminder = reminderFixture.copy(
            time = LocalTime.of(14, 0), // 2:00 PM (same as current time)
            checkedDays = CheckedDays.none().copy(tuesday = true, thursday = true) // Tue & Thu enabled
        )
