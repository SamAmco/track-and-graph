...
fun finishProto (proto: PossibleIntervalProto, y_min: Double, y_max: Double, fixedBounds: Boolean): PossibleInterval {
...
fun finishProtoDynamicBounds (proto: PossibleIntervalProto, y_min: Double, y_max: Double ): PossibleInterval {
...
fun finishProtoFixedBounds (proto: PossibleIntervalProto, y_min: Double, y_max: Double) : PossibleInterval{
    ...
    if (y_range.div(proto.interval).rem(1) != 0.0 ) return PossibleInterval(0.0, false, 999, 0.0, 0.0, 0.0, 1.0)
...
fun getYParameters(y_min: Double, y_max: Double, time_data: Boolean,
    ...
    fun convertToOutput(interval: PossibleInterval) = YAxisParameters(
            StepMode.SUBDIVIDE, interval.n_lines.toDouble(), interval.bounds_min, interval.bounds_max)

    try {
        val parameters = getYParametersInternal(y_min, y_max, time_data, fixedBounds)
        return convertToOutput(parameters)
    } catch (e: Exception) {
        if (throw_exc_if_non_found) throw e
...
fun getYParametersInternal(y_min: Double, y_max: Double, time_data: Boolean,
                   fixedBounds: Boolean) : PossibleInterval {
    val MIN_INTERVALS = 6
    ...
    if (reasonable_intervals.isEmpty()) {
        throw Exception("No solution found! No intervals passed the initial filtering. ymin: $y_min, ymax: $y_max")
    }
    ...
    throw Exception("No solution found! No intervals passed the range_used check. ymin: $y_min, ymax: $y_max")
}
