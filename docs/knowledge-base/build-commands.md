...
## Graph Screenshot Regression Tests

Graph visual regression previews live under the `regression/graphs` package in the screenshot-test source set. Unlike the Play Store and tutorial previews, their FOSS debug reference PNGs are durable test inputs and should remain in source control. The colocated screenshot-test README records this distinction and the update/validation commands.

The suite calls the production list-mode graph composables with deterministic view-data fixtures. Graph previews fix only a representative in-card content width and leave height unspecified, so the golden bounds follow the composable's wrapped height; a high-cardinality legend case guards vertical expansion. It keeps a bounded set of cases per graph type covering materially different rendering risks. Structural cases render once in English, while selected cases covering the distinct localized date tiers, histogram units, percentages, and empty-label text use a multi-preview annotation for all supported locales. Use `:app:updateFossDebugScreenshotTest` only for an intentional baseline replacement; use `:app:validateFossDebugScreenshotTest` for verification. The FOSS update task also renders the existing asset-generation previews because they share the source set, but the package path in the reference tree keeps the committed regression goldens identifiable.

The graph preview environment initializes AndroidThreeTen and fixes the host timezone to UTC itself. Do not rely on the Play Store or tutorial previews running first: filtered graph-only validation must work in isolation.
