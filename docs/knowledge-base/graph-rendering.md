...
X ticks are selected from actual data-point timestamps, including irregular series, rather than fixed temporal subdivisions. Eligible interior ticks are anchored to their index in the complete, sorted timestamp sequence rather than whichever point is currently at the viewport edge. Tick strides use powers of two: panning moves the same interior labels through the viewport, while zooming in retains coarse labels and adds intermediate data-point labels when space permits. The first and last visible data points are also always eligible. The plot reserves enough start space for the first label and prioritizes it in the left-to-right overlap pass; the last label is retained only when it does not collide with the label to its left. Rotated labels track their left and right extents separately: the first label's true left edge aligns with the Canvas edge, avoiding both clipping and redundant inset inside the card padding. During a pan, boundary labels therefore move with their data point and change only as points enter or leave the viewport, while interior labels remain anchored. The selector measures one real candidate near the middle of the viewport to estimate capacity, then compares upper bounds for measuring every visible timestamp versus measuring only stride-eligible timestamps. Because that estimate is itself a real candidate, the direct path does not pay for a throwaway measurement. The chosen candidates then use their exact measured bounds in the final -28-degree overlap pass, so an imperfect initial estimate cannot cause overlaps. Formatting is based on the visible duration: seconds and minutes use clock formats, day-scale labels use `dd MMM`, and long ranges use the compact, unambiguous abbreviated-year form `MMM’yy` (for example, `Mar’26`). The named-month formatters use `Locale.ENGLISH` deliberately so every month occupies exactly three characters (`Sep`, never locale-dependent `Sept`). Recalculate both axes when the full-screen viewport changes.
